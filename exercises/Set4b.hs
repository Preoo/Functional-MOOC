...
countHelper :: Num a => Maybe b -> a -> a
countHelper el acc = acc + maybe 1 (const 0) el
...
slHelper n acc = (sum' :: Double, count :: Int) where
    sum' = n + fst acc
    count = 1 + snd acc
...
largestHelper n acc = filter (== x) m where
                        x = maximum m
                        m = n : acc
...
lastHelper n acc = case acc of Just x -> Just x
                               Nothing -> Just n