...
capitalizeString (s:sx) = toUpper s:sx

capitalize :: String -> String
capitalize s = unwords . map capitalizeString $ words s
...
whileRight f x = case f x of (Left x) -> x
...
joinToLength n x = [ab | a <- x, b <- x, let ab = a ++ b, length ab == n]
...
interpreter = loop [] 0 0
    where loop out x y [] = reverse out
          loop out x y (c:cs) = case c of "up"     -> loop out x (y+1) cs
                                          "down"   -> loop out x (y-1) cs
                                          "left"   -> loop out (x-1) y cs
                                          "right"  -> loop out (x+1) y cs
                                          "printX" -> loop (show x:out) x y cs
                                          "printY" -> loop (show y:out) x y cs
