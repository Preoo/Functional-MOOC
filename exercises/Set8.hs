...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) =
  Color (avg r1 r2) (avg g1 g2) (avg b1 b2) where avg i j = (i+j) `div` 2
...
rectangle x0 y0 w h = Shape (\(Coord x y) -> (x0 <= x && x < x0+w)
                                          && (y0 <= y && y < y0+h))
------------------------------------------------------------------------------
...
paintSolid c (Shape s) (Picture base) = Picture g
...
paint (Picture patr) (Shape s) (Picture base) = Picture g
  where g coord | s coord   = patr coord
                | otherwise = base coord
------------------------------------------------------------------------------
...
instance Transform Flip where
  apply FlipX  (Picture p) = Picture (p . \(Coord x y) -> Coord (-x) y)
  apply FlipY  (Picture p) = Picture (p . \(Coord x y) -> Coord x (-y))
  apply FlipXY (Picture p) = Picture (p . \(Coord x y) -> Coord y x)
------------------------------------------------------------------------------
...
-- What a mess (._.') hardcoded averaging for 5 pixel colors
instance Transform Blur where
  apply Blur (Picture p) = Picture g
    where g (Coord x y) = blur (p (Coord x y)) 
                               (p (Coord (x+1) y)) (p (Coord (x-1) y)) 
                               (p (Coord x (y+1))) (p (Coord x (y-1)))

          blur (Color rc gc bc) (Color ru gu bu) (Color rd gd bd) 
               (Color rl gl bl) (Color rr gr br) 
               = Color ((rc+ru+rd+rl+rr) `div` 5) ((gc+gu+gd+gl+gr) `div` 5) 
                       ((bc+bu+bd+bl+br) `div` 5)
...
instance Transform BlurMany where
  apply (BlurMany n) p | n > 0 = apply (BlurMany (n-1)) (apply Blur p)
                       | otherwise = p
