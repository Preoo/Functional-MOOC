...
treeMax Empty = 0
treeMax (Node i l r) = max i $ max (treeMax l) (treeMax r)
...
allValues condition (Node i l r) = condition i
...
mapTree f t = case t of Empty        -> Empty
                        (Node i l r) -> Node (f i) (mapTree f l) (mapTree f r)
...
cull _ Empty = Empty
cull val (Node i l r) = if val == i 
                        then Empty 
                        else Node i (cull val l) (cull val r)
...
isOrdered (Node i l r) = allValues (<i) l && allValues (>i) r 
...
walk [] t = case t of Empty      -> Nothing
                      Node i _ _ -> Just i
walk (s:ss) (Node _ l r) = case s of StepL -> walk ss l
                                     StepR -> walk ss r
...
set _ _ Empty = Empty
set [] val (Node i l r) = Node val l r
set path val (Node i l r) = case p of StepL -> Node i (set rest val l) r
                                      StepR -> Node i l (set rest val r)
                            where p:rest = path
...
-- shortcircuit if target is in root
-- else call into recursive path building helper function and return path if not empty
search :: Eq a => a -> Tree a -> Maybe [Step]
search _ Empty = Nothing
search target (Node i l r) 
  | target == i = Just []
  | otherwise = case search' target (Node i l r) [] of []   -> Nothing
                                                       path -> Just path

-- looks awful to build pathlike this instead of using Maybe's 
-- but it best I came up with in a hurry
search' :: Eq a => a -> Tree a -> [Step] -> [Step]
search' _ Empty _ = []
search' t (Node i l r) path = if t == i 
                              then path 
                              else search' t l (path ++ [StepL]) ++ search' t r (path ++ [StepR])
