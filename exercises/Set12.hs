...
instance Functor List where
  fmap _ Empty          = Empty
  fmap f (LNode n rest) = LNode (f n) (fmap f rest)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode n m rest) = TwoNode (f n) (f m) (fmap f rest)
...
-- both will pass tests
-- count c = foldl' (\sum_ i -> if i == c then sum_+1 else sum_) 0
count c = getSum . foldMap (Sum . fromEnum . (== c))
...
inBoth x y = foldr' (\i arr -> if i `elem` y then i:arr else arr) [] x
...
instance Foldable List where
  ...
  foldMap f (LNode x rest) = f x <> foldMap f rest
...
instance Foldable TwoList where
  ...
  foldMap f (TwoNode x y rest) = f x <> f y <> foldMap f rest
...
instance Functor Tree where
  fmap _ Leaf = Leaf
  fmap f (Node c l r) = Node (f c) (fmap f l) (fmap f r)
