...
-- remove it?
-- Ans: List values would be missing equals operation for instance a. 
--      I'd have to implement Eq for all instances of a.
--      Using Eq a constraint autogenerates these instances
...
instance Price Egg where
  price a = case a of ChickenEgg   -> 20
                      ChocolateEgg -> 30
...
simplify (RationalNumber a b) = RationalNumber c d where
  c = a `div` g
  d = b `div` g
  g = gcd a b
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 a = a
  stepMany c a = stepMany (c-1) (step a)
