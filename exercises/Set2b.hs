...
leftpad s i
-- n > 0 = leftpad (" " ++ s) i
-- old version above, new version uses shorthand >> == concat $ replicate i s
-- to generate prefix with required length
    | n > 0 = ([1..n] >> " ") ++ s
    | otherwise = s
    where n = i - length s
...
countdown i = countdown' "" i where
    countdown' s 0 = "Ready! " ++ s ++ "Liftoff!"
    countdown' s i = countdown' (s ++ show i ++ "... ") (i-1)
...
smallestDivisor n = head (filter (\k -> n `mod` k == 0) [2..n])
