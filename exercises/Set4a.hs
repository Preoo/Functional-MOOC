...
sortByLongestAndHead :: Ord a => [a] -> [a] -> Ordering
sortByLongestAndHead as bs
    | al < bl = GT
    | al > bl = LT
    | al == bl = compare (head as) (head bs)
    where al = length as
          bl = length bs

longest :: Ord a => [[a]] -> [a]
longest = minimumBy sortByLongestAndHead
...
incrementKey key = map (\p -> if fst p == key then (fst p, snd p + 1) else p)
...
freqs = foldr count Map.empty
    where count e acc = Map.alter updateFreq e acc
          updateFreq v = case v of Nothing -> Just 1
                                   Just n  -> Just (n+1)
...
transfer from to amount bank
    | amount <= 0 = bank
    | otherwise =
        case (Map.lookup from bank, Map.lookup to bank)
        of (Just f, Just t) -> if f >= amount
                               then Map.insert from (f-amount)
                                    . Map.insert to (t+amount)
                                    $ bank
                               else bank
           _ -> bank
...
maxIndex = fst . go . assocs where
    go [x] = x
    go (x:y:xs) = if snd x > snd y 
                  then go (x:xs) 
                  else go (y:xs)